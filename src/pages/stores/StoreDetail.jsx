...
export default function StoreDetail() {
  ...
  const { getCategoriesById, categoriesById } = useCategoriesStore(
    (state) => state
  );
  ...
  const columnProduct = [
      ...
      render: (_, record) =>
        record?.skus?.map((item, index) => (
          <div key={index}>
            {index > 0 && <Divider />}
            <p>
              <span>{item.price.currency}</span> {item.price.original_price}
            </p>
          </div>
        )),
    },
    ...
    },
    {
      ...
      render: (create_time) => (
        <span>{formatDate(create_time, "DD/MM/YY, h:mm:ss a")}</span>
      ),
    },
    {
      ...
      render: (update_time) => (
        <span>{formatDate(update_time, "DD/MM/YY, h:mm:ss a")}</span>
      ),
    },
    {
      title: "Hành động",
      ...
      render: (_, record) => (
        <>
          ...
          <Button
            type="primary"
            onClick={() => navigate(`/shops/${store.id}/products/${record.id}`)}
            className="ml-3"
          >
            Xem
          </Button>
      ...
      <div className="p-10">
            ...
            <Row>
              ...
              <Col span={12}>
                <Row className="items-center gap-[4px] justify-start mt-3 break-words flex-nowrap">
                  ...
                  <Col className="font-medium text-[#21409A]">
                    {address ? address : "Hiện không có địa chỉ"}
                  </Col>
                </Row>
              </Col>
              <Col span={12}>
              ...
              <Col key={warehouse.warehouse_id} span={12}>
                <Card className="relative">
                  ...
                  <Row className="items-center gap-[4px] justify-start mt-3 break-words flex-nowrap">
                    <Col span={4}>Địa chỉ:</Col>
                    <Col className="font-medium text-[#21409A]">
                      {warehouse?.warehouse_address?.full_address}
                    </Col>
                  </Row>
                  ...
                  <Row className="items-center gap-[4px] justify-start mt-3 break-words flex-nowrap">
                    ...
                    <Col className="font-medium text-[#21409A]">
                      {warehouse?.warehouse_address?.region}
                    </Col>
                  </Row>
                  ...
                  <Row className="items-center gap-[4px] justify-start mt-3 break-words flex-nowrap">
                    ...
                    <Col className="font-medium text-[#21409A]">
                      {warehouse?.warehouse_address?.state}
                    </Col>
                  </Row>
                  ...
                  <Row className="items-center gap-[4px] justify-start mt-3 break-words flex-nowrap">
                    ...
                    <Col className="font-medium text-[#21409A]">
                      {warehouse?.warehouse_address?.zipcode}
                    </Col>
                  </Row>
                </Card>
              </Col>
        ...
        <div className="mb-10">
          <h3 className="text-[16px] font-semibold mb-3">
            ...
            <span className="font-bold text-[#1677ff]">
              ({category_list?.length > 0 ? category_list?.length : 0})
            </span>
          </h3>
          <Table
            ...
            bordered
            dataSource={
              category_list && category_list?.length ? category_list : []
            }
          />
        </div>
        ...
        <div>
          <div className="flex justify-between mb-3">
            ...
            <Button
              type="primary"
              onClick={() => navigate(`/shops/${store.id}/add-many-products`)}
            >
              Thêm hàng loạt
            </Button>
          </div>
