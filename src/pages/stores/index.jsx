...
import { Button, Drawer, Layout, Modal, Space, Table, message } from "antd";
...
const Stores = () => {
  ...
  const copyToClipboard = (token) => {
    ...
    // const copyContent = contentRef.current.innerText;
    navigator.clipboard
  ...
  const storesTable = [
    ...
    },
    {
      ...
      key: "shop_code",
      render: (code, seller) => (
        <p className="text-[#0e2482] font-medium cursor-pointer">{code}</p>
      ),
    },
    {
      title: "Grant type",
    ...
    <Layout.Content className="mt-4 px-5">
      ...
      >
        {!app_key && !code ? (
          ...
          <StoreForm app_key={app_key} code={code} />
        )}
      </Modal>
    </Layout.Content>
  );
