...
function ProductEdit() {
  ...
  const priceDataForm: number | string =
    productById?.skus?.length === 1 ? formatNumber(productById?.skus[0]?.price?.original_price ?? '') : '';
  const availableDataForm: number | string =
    productById?.skus?.length === 1 ? formatNumber(productById?.skus[0]?.stock_infos[0]?.available_stock ?? '') : '';
  const skuDataForm: number | string =
    productById?.skus?.length === 1 ? formatNumber(productById?.skus[0]?.seller_sku ?? '') : '';
  const warehouse_id: string =
    productById?.skus?.length === 1 ? productById?.skus[0]?.stock_infos[0]?.warehouse_id : '';
  const available_stock: number =
    productById?.skus?.length === 1 ? productById?.skus[0]?.stock_infos[0]?.available_stock : '';
