...
function Orders() {
  ...
  const columns = [
      ...
      onCell: (record, index) => {
        const rowSpanData = orderList.filter(
          (item) =>
            ...
            item.package_list[0].package_id === record.package_list[0].package_id,
        );
    ...
    },
    {
      ...
      key: 'order_code',
      render: (_, record) => (
        <Link to={`/shops/${shopId}/order/${record?.order_id}`} state={{ orderData: record }} className="font-medium">
          {record?.order_id}{' '}
